  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex flex-col">
      ...
      <header className="border-b border-zinc-800 px-6 py-3 flex items-center justify-between shrink-0">
        <a href={`/u/${data.username}`} className="flex items-center gap-2 transition-opacity hover:opacity-80">
        ...
        <a
          href={isOwner ? "/dashboard" : `/u/${data.username}`}
          className="text-xs text-zinc-600 hover:text-zinc-400 transition-colors"
        >
          {isOwner ? "← Panele Dön" : "← Profile Dön"}
        </a>
      </header>
