...
async function githubFetch(path: string, token: string) {
  ...
  return { json: await res.json(), remaining: Number(res.headers.get("x-ratelimit-remaining") ?? 5000) };
}
  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        const rateRemaining = await getRateLimit(token);
        if (rateRemaining < 100) {
          send(controller, "error", {
            message: `GitHub API limiti yetersiz: ${rateRemaining} istek kaldı. Biraz bekleyip tekrar dene.`,
          });
          controller.close();
          return;
        }
        send(controller, "progress", { step: "start", message: `API limiti: ${rateRemaining} istek kaldı` });

        const { data: dbUser } = await supabaseAdmin
          .from("users").select("id, last_synced_at").eq("username", username).single();

        if (!dbUser) throw new Error("Kullanıcı bulunamadı");
        const userId = dbUser.id;

        // J.2 — Incremental sync: son sync'ten bu yana, yoksa 1 yıl
        const isIncremental = !!dbUser.last_synced_at;
        const since = isIncremental
          ? new Date(dbUser.last_synced_at!).toISOString()
          : new Date(Date.now() - 365 * 24 * 60 * 60 * 1000).toISOString();

        if (isIncremental) {
          const sinceLabel = new Date(dbUser.last_synced_at!).toLocaleString("tr-TR");
          send(controller, "progress", { step: "start", message: `Artımlı sync: ${sinceLabel} tarihinden bu yana` });
        }
        ...
        // 2. Diller — sadece son sync'ten sonra güncellenen repolar için (J.2)
        const reposForLangs = isIncremental
          ? ownRepos.filter((r) => new Date(r.updated_at) > new Date(since))
          : ownRepos;
        send(controller, "progress", { step: "langs", message: `Dil istatistikleri çekiliyor (${reposForLangs.length} repo)...` });
        for (const repo of reposForLangs) {
          try {
        ...
        // 3. Commitler — `since` parametresi ile sadece yeniler (J.2)
        ...
        for (const repo of ownRepos) {
          ...
          // J.3 — Döngü içi rate limit kontrolü: 50'nin altına düşerse dur
          const remNow = await getRateLimit(token);
          if (remNow < 50) {
            send(controller, "progress", { step: "commits", message: `Rate limit: ${remNow} kaldı, commitler tamamlanamadı` });
            break;
          }
        ...
        // 4. PR'lar — since parametresiyle (J.2)
        send(controller, "progress", { step: "prs", message: "Pull Request'ler çekiliyor..." });
        for (const repo of ownRepos) {
          const dbRepo = repoMap.get(repo.id);
          if (!dbRepo) continue;
          try {
            const sinceParam = `&sort=created&direction=desc${isIncremental ? "" : ""}`;
            const prs = await fetchAllPages<{
              ...
              user: { login: string };
            }>(`/repos/${repo.full_name}/pulls?state=all${sinceParam}`, token, 50);

            // Incremental'da sadece since'den sonrakiler
            const myPrs = prs
              .filter((pr) => pr.user?.login === username)
              .filter((pr) => !isIncremental || new Date(pr.created_at) > new Date(since));
        ...
        const { data: allCommits } = await supabaseAdmin
          ...
          .gte("committed_at", since);  // J.2: sadece since'den sonraki commitler
        ...
        send(controller, "done", {
          message: isIncremental
            ? "Artımlı senkronizasyon tamamlandı!"
